   ...
   def transform(df):
      ...
      for index, row in df.iterrows():
         ...
         try:
            ...
            hour_start2 = None
         if hour_start1 >= 13 and hour_start2 == None:
...
if st.session_state["authentication_status"]: # successful login
   with c1:
      authenticator.logout('Logout', 'main')
   c2.write(f'Welcome *{st.session_state["name"]}*')
   main()

elif st.session_state["authentication_status"] is False: # unsuccessful login
   c1.error('Username/password is incorrect')
   try:
      with c2:
         username_forgot_pw, email_forgot_password, random_password = authenticator.forgot_password('Forgot password')
      if username_forgot_pw:
         c2.success('New password sent securely')
         # Random password to be transferred to user securely
         with open('config.yaml', 'w') as file:
            yaml.dump(config, file, default_flow_style=False)

      else:
         c2.error('Username not found')
   except Exception as e:
      c2.error(e)
elif st.session_state["authentication_status"] is None: # no login attempt
